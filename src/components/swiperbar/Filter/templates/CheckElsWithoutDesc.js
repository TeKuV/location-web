...
const CustomCheckBox = ({typo, datas, setDatas}) => {
    const [checked, setChecked] = useState(search(datas, typo) !== -1)
    ...
    return (
        <FormControlLabel sx={{width: 310}} control={<Checkbox size={'medium'} sx={{color: 'common.black'}} checked={checked} onChange={(event) => handleChange(event.target.checked, typo)} />} label={
                <Typography sx={{fontSize: 17 }}>{typo}</Typography>
        } />
    )
}

const CheckElsWithoutDesc = ({datas, setDatas, typos}) => {
...
export default CheckElsWithoutDesc;